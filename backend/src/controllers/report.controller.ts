    ...
    try {
      ...
      const data = charges.map(c => {
        ...
        return {
          ...
          "Tipo": c.type,
          "Monto Base (REF)": Number(c.amount),
          "Mora (REF)": Number(c.moraAmount),
          "Total Cuota (REF)": Number(c.amount) + Number(c.moraAmount),
          "Pagado (REF)": Number(c.amountPaid),
          "Adeudado (REF)": amountOwed,
          "Tasa de Cobro": c.currency ?? "EUR",
